...
function GroupCreation() {
  ...
  async function createNewGroup() {
    ...
    let groupName = extractedContent?.find(item => item.type === 'text')?.value?.trim();
    // Define the group data to be stored in the database
    const groupData = {
      profileInfo: {
        ...
        bgColor: getColorForName(groupName),
        // Background color when no profile pic, based on the first letter of the group name
        description: JSON.stringify([{ type: 'text', value: "Your Group" }]), // Default description
      },
    ...
    const commonChatData = {
      ...
      receiversInfo: selectedUsers?.map(memberData => ({
        status: 'sending', // Initial status before message is delivered
        deliveredTime: null,
        seenTime: null,
        receiverID: memberData?._id, // Recipient user ID
      })),
      deletedByUsers: [],
      starredByUsers: [],
      keptByUsers: [],
    ...
    // Chat message when the group is created
    const groupCreatingChatData = {
      ...commonChatData,
      customID: generateUniqueID("CHAT"), // Generate a unique ID for the chat message
      chatType: 'system-group-creating',
      sentTime: new Date().toISOString(),
      text: null //it will be updated after group creation in server side
    };

    // Chat message when members are added
    const groupAddingChatData = {
      ...commonChatData,
      customID: generateUniqueID("CHAT"), // Generate another unique ID for this chat
      chatType: 'system-adding-member-group',
      sentTime: new Date().toISOString(),
      text: null //it will be updated after group creation in server side
    };

    // Manage the group chat tab
    let tabInfo = {
      tabType: "group",
      tabID: null,
      recentTime: groupAddingChatData?.sentTime, // Use the sentTime of the groupAddingChatData
      clearingTime: "", // No clearing time for group tab at creation
      isArchived: false, // Group tab are never archived by default
      isPinned: false, // Group tab are never pinned by default
      disappearingTime: "Off", // Group tab are never disappearing by default
    };
    // Send group creation event to the server
    ...
    //adding tabInfo will be handled when group is created , 
    // go to - at handleGroupCreateSuccess , line : 1532 in UserContext.jsx
  };
        ...
        <div style={{ backgroundColor: "rgb(245,247,251)" }} className={`${activeDarkMode ? "darkModeBg1" : ''} overlayInner relative h-full m-auto text-gray-900`}>
          ...
          <div className='relative w-full h-full flex flex-col'>
            <React.Fragment>
              ...
              {/* show panel for new group and other details */}
              {
                showInfoPanel &&
                <>
                  {
              ...
              {/* show emoji container when need to add emoji in group name */}
              {
                showEmojiContainer &&
